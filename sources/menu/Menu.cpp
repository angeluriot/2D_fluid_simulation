...
void Menu::display()
{
	if (visible)
	{
		...
		Simulator::resolution *= 100.f;

		ImGui::NewLine();
		ImGui::Text("The resolution of the grid:");
		if (ImGui::SliderFloat("##resolution", &Simulator::resolution, 5.f, 100.f, "%.0f %%"))
		{
			Simulator::resolution /= 100.f;
			Simulator::reset();
		}
		...
		ImGui::NewLine();
		ImGui::Text("The number of iterations:");
		ImGui::SliderInt("##viscous_precision", &Simulator::iterations, 10, 100);
